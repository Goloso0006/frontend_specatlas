...
- `POST /api/requirements/convert` -> convierte texto a `RequirementDTO`
- `POST /api/requirements/save` -> guarda requisito
- `GET /api/requirements/search?query=...` -> busqueda semantica (`SearchResponse[]`)
- `POST /api/requirements/check-duplicates` -> `ApiResponse<DuplicateMatchResponse[]>`
- `GET /api/requirements/{id}/impact` -> impacto en grafo
- `POST /api/requirements/dependency?fromId=...&toId=...`
...
| **Proyectos** | `POST` | `/api/projects` | `{ name, description, ownerId, status }` | `{ projectId, name, status }` | ProjectsPage | Si |
|  | `GET` | `/api/projects/{id}` | — | `{ projectDetails }` | ProjectsPage | Si |
|  | `GET` | `/api/projects/user/{ownerId}` | — | `{ projectsList }` | ProjectsPage | Si |
|  | `PUT` | `/api/projects/{id}` | `{ name, description, status }` | `{ updatedProject }` | ProjectsPage | Si |
|  | `DELETE` | `/api/projects/{id}` | — | `204 No Content` | ProjectsPage | Si |
| **Requisitos** | `POST` | `/api/requirements/convert` | `{ text, projectId }` | `{ requirementDTO }` | RequirementsPage | Si |
|  | `POST` | `/api/requirements/save` | `{ RequirementDTO }` | `{ savedRequirement }` | RequirementsPage | Si |
|  | `GET` | `/api/requirements/project/{projectId}` | — | `{ requirementsList }` | RequirementsPage | Si |
|  | `GET` | `/api/requirements/search` | `?query=...` | `{ searchResults }` | RequirementsPage | Si |
|  | `POST` | `/api/requirements/check-duplicates` | `{ projectId, title, description }` | `{ duplicatesList }` | RequirementsPage | Si |
|  | `GET` | `/api/requirements/{id}/impact` | — | `{ impactDetails }` | RequirementsPage | Si |
|  | `POST` | `/api/requirements/dependency` | `?fromId=...&toId=...` | `200 vacio` | RequirementsPage | Si |
|  | `GET` | `/api/requirements/{id}/conflicts` | — | `{ conflicts }` | RequirementsPage | Si |
| **Grafo / Relaciones** | `GET` | `/api/graph/impact/{id}` | — | `{ impactGraphData }` | RequirementsPage | Si |
|  | `POST` | `/api/graph/infer-relations/{projectId}` | — | `{ relationsInferred }` | RequirementsPage | Si |
| **Diagramas (Clases)** | `POST` | `/api/diagrams/manual` | `{ projectId, name, sourceJson, plantUmlCode }` | `{ diagramResponse }` | DiagramEditorPage | Si |
|  | `POST` | `/api/diagrams/class/auto/{projectId}` | — | `{ autoGeneratedDiagram }` | DiagramEditorPage | Si |
|  | `POST` | `/api/diagrams/{diagramId}/plantuml` | `{ plantUmlCode }` | `{ updatedPlantUmlCode }` | DiagramEditorPage | Si |
|  | `GET` | `/api/diagrams/{diagramId}/export/puml` | — | `{ pumlFileContent }` | DiagramEditorPage | Si |
|  | `GET` | `/api/diagrams/{diagramId}/export/txt` | — | `{ txtFileContent }` | DiagramEditorPage | Si |
| **Diagramas (Casos de Uso)** | `POST` | `/api/diagrams/use-case/manual` | `{ projectId, name, sourceJson, plantUmlCode }` | `{ useCaseDiagramResponse }` | DiagramEditorPage | Si |
|  | `POST` | `/api/diagrams/use-case/auto/{projectId}` | — | `{ autoGeneratedUseCaseDiagram }` | DiagramEditorPage | Si |
| **Reglas de Validacion** | `GET` | `/api/validation-rules/project/{projectId}` | — | `{ validationRulesList }` | ValidationRulesPage | Si |
|  | `POST` | `/api/validation-rules` | `{ ValidationRuleRequest }` | `{ createdValidationRule }` | ValidationRulesPage | Si |
|  | `PUT` | `/api/validation-rules/{id}` | `{ ValidationRuleRequest }` | `{ updatedValidationRule }` | ValidationRulesPage | Si |
|  | `DELETE` | `/api/validation-rules/{id}` | — | `204 No Content` | ValidationRulesPage | Si |
...
================Plan acción Jueves 30 de Abril =================
1.Primero conecta el frontend con el backend y valida los endpoints.
