...
export function FunctionalRequirementManualPage() {
  ...
  const [showInstructions, setShowInstructions] = useState(true) // ISO: auto descriptivo

  // ── Handlers ────────────────────────────────────────────────────────────

  const loadRequirements = async (activeProjectId: string) => {
        ...
        {showInstructions && (
          <div className="mt-3 flex flex-wrap items-center gap-x-6 gap-y-2 text-xs text-[var(--color-text-muted)] bg-[var(--color-bg-card)] border border-[var(--color-border)] rounded-lg p-3">
            ...
            <div className="flex items-center gap-1.5">
              ...
              <span>Use el botón <span className="font-semibold text-[var(--color-text-primary)]">+ Nuevo</span> para agregar requerimientos.</span>
            </div>
          </div>
        )}
      </header>

      {/* ==================== MAIN CONTENT: TABLA Y DETALLE ==================== */}
